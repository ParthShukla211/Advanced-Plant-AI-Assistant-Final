...
- **🧩 Developer Tools:** Access tools to clear caches, reset analytics, and inspect the session state.
    ...
    if user_question:
        ...
        with st.chat_message("assistant"):
            # Use unique container keys based on run_id to force recreation
            container_key = f"container_{st.session_state['run_id']}"
            
            # Create a single container that will be completely replaced
            main_container = st.container(key=container_key)
